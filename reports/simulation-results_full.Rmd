...
p0 <- data.frame(Control = betas_orig[1] + betas_orig[2]*0:8,
  ...
  pivot_longer(cols = 1:3,
               values_to = "log10(TTP)",
               names_to = "Original Regimen") %>% 
  ggplot(aes(x = weeks, y = 10^(`log10(TTP)`), col = `Original Regimen`)) + 
  geom_line() + 
...
p00 <- data.frame(Control = betas[1] + betas[2]*0:8,
                  Regimen1 = betas[1] + betas[2]*0:8,
                  Regimen2 = betas[1] + betas[2]*0:8,
                  Regimen3 = betas[1] + betas[2]*0:8,
                  Regimen4 = betas[1] + betas[2]*0:8,
                  weeks = 0:8) %>% 
  pivot_longer(cols = 1:5,
               values_to = "log10(TTP)",
               names_to = "Simulated Regimen") %>% 
  ggplot(aes(x = weeks, y = 10^(`log10(TTP)`), col = `Simulated Regimen`, lty = `Simulated Regimen`)) + 
  geom_line() + 
  scale_y_log10("TTP in days, log10 scale",
                breaks = c(5,7,10,15,20,25,35),
                labels = c(5,7,10,15,20,25,35)) + 
  ...
  coord_cartesian(ylim = c(7,35)) + 
  ggtitle("No Winners condition", subtitle = c("0%, 0%, 0%, 0%")) + 
  theme_bw()
...
```{r}
ggsave(filename = here("graphs", "simulation-settings", "2023-09-11_ttp-simulation-settings.png"),
       device = "png",
       height = 9,
       width = 7,
       units = "in")
...
p_outcomes <- bind_rows(mutate(s95.sit1.20, nk = 20),
...
```{r eval= FALSE}
ggsave(filename = here("graphs", "submission", "fig-03_relapse-thresholds.tiff"),
       device = "tiff",
       height = 6,
       width = 6,
       units = "in")

p_outcomes + ggtitle("Dufault Fig. 3 (Top)")
ggsave(filename = here("graphs", "submission", "fig-03_relapse-thresholds_annotated.tiff"),
       device = "tiff",
       height = 6,
       width = 6,
       units = "in")
...
load(here("data", "analyzed", "target-product-profile", "2023-09-12_tpp-decisions.RData"))
```
...
d_null <- decisions$decisions_null %>% 
  filter(Regimen == "Regimen 1") %>% 
  mutate(Regimen = case_when(Regimen == "Regimen 1" ~ "0%"))
...
d_full %>% 
  ...
  group_rows("Slope, %: 0%", start_row = 6, end_row = 10) %>% 
  group_rows("Slope, %: 10%", start_row = 11, end_row = 15) %>%
  group_rows("Slope, %: 20%", start_row = 16, end_row = 20) %>% 
  group_rows("Slope, %: 30%", start_row = 21, end_row = 25) %>% 
  group_rows("Slope, %: 40%", start_row = 26, end_row = 30)
```
...
```{r eval=FALSE}
ggsave(filename = here("graphs", "submission", "fig-04_barplot-tpp-decisions.tiff"),
       device = "tiff",
       height = 6,
       width = 9,
       units = "in")
...
ggsave(filename = here("graphs", "submission", "fig-04_barplot-tpp-decisions_annotated.tiff"),
       device = "tiff",
       height = 6,
       width = 9,
       units = "in")
...
```{r }
load(here("data", "analyzed", "ranking", "2023-09-12_ranking-probabilities.RData"))
load(here("data", "analyzed", "ranking", "2023-09-12_compare-v-control.RData"))

comparisons <- comparisons %>% 
  mutate(condition = case_when(condition == "High" ~ "4 Winners",
                              condition == "Null" ~ "0 Winners",
                              condition == "Even" ~ "1 Winner",
                              TRUE ~ "2 Winners"))
```
...
p30 <- comparisons %>%
  filter(nk == "nk30") %>%
  mutate(condition = factor(condition, levels = c("0 Winners", "1 Winner", "2 Winners", "4 Winners"))) %>% 
...
null_stat <- ranking_probabilities$null_probabilities %>% 
  group_by(sim, nk) %>% 
  mutate(top = as.numeric(p.subset.1 == max(p.subset.1))) %>% 
  group_by(regimen, nk) %>% 
  summarise(n_top = sum(top)) %>% 
  mutate(condition = "0 Winners")
...
bind_rows(null_stat,
  ...
  mutate(theta = paste0("$\\theta_{(", 6 - as.numeric(str_remove(regimen, "arm")), ")}$"),
         condition = factor(condition, levels = c("0 Winners", "1 Winner", "2 Winners", "4 Winners"))) %>%
...
```{r}
p0 <- mutate(ranking_probabilities$null_probabilities, condition = "0 Winners") %>% 
...
p0 <- ranking_probabilities$null_probabilities %>% 
...
```{r}
temp0 <- ranking_probabilities$null_probabilities %>% 
  group_by(sim, nk) %>% 
...
p1 <- bind_rows(temp0, temp1, temp2, temp3) %>% 
...
data_shape_function <- function(relapse_data, relapse_regimen, true_slopes, tpp_data, ranking_data, nk_choice){
  # Relapses
  ...
  out1 <- full_join(
    ...
    mutate(true.slope = true_slopes[1],
           true.relapse = "sub-optimal",
           nk = nk_choice)
  out2 <- full_join(
    full_join(r1, tpp2), rank2) %>% 
    mutate(true.slope = true_slopes[2],
           true.relapse = "sub-optimal",
           nk = nk_choice)
  out3 <- full_join(
    full_join(r1, tpp3), rank3) %>% 
    mutate(true.slope = true_slopes[3],
           true.relapse = "sub-optimal",
           nk = nk_choice)
  out4 <- full_join(
    full_join(r1, tpp4), rank4) %>% 
    mutate(true.slope = true_slopes[4],
           true.relapse = "sub-optimal",
           nk = nk_choice)
...
```{r}
#| eval: false
ggsave(filename = here("graphs", "submission", "fig-02_panel-oc-full-framework.tiff"),
       device = "tiff",
       height = 8,
       width = 8,
       units = "in")
...
ggsave(filename = here("graphs", "submission", "fig-02_panel-oc-full-framework_annotated.tiff"),
       device = "tiff",
       height = 8,
       width = 8,
       units = "in")
...
d20 <- data_shape_function(relapse_data = s95.sit1.20,
                           relapse_regimen = "2", # Sub-optimal
                           tpp_data = decisions$decisions_even,
                           ranking_data = ranking_probabilities$even_probabilities,
                           true_slopes = c("10%", "20%", "30%", "40%"),
                           nk_choice = 20)
d30 <- data_shape_function(relapse_data = s95.sit1.30, 
                           relapse_regimen = "2", # Sub-optimal
                           tpp_data = decisions$decisions_even,
...
p_suboptimal <- bind_rows(d20, d30, d40)  %>% 
  mutate(decision = case_when((n_relapse_interim1 < 2 & Decision == "GO" & p.subset.2 >= 0.5) ~ "GO",
                              ((n_relapse_interim1 >= 2) | (Decision == "NO-GO")) ~ "NO-GO",
                              TRUE ~ "Continue")) %>% 
  mutate(decision = factor(decision, levels = c("NO-GO", "Continue", "GO"))) %>% 
...
p_minimal <- bind_rows(d20, d30, d40)  %>%
  mutate(decision = case_when((n_relapse_interim1 < 2 & Decision == "GO" & p.subset.2 >= 0.5) ~ "GO",
                              ((n_relapse_interim1 >= 2) | (Decision == "NO-GO")) ~ "NO-GO",
                              TRUE ~ "Continue")) %>% 
  mutate(decision = factor(decision, levels = c("NO-GO", "Continue", "GO"))) %>% 
...
p_desirable <- bind_rows(d20, d30, d40)  %>% 
  mutate(decision = case_when((n_relapse_interim1 < 2 & Decision == "GO" & p.subset.2 >= 0.5) ~ "GO",
                              ((n_relapse_interim1 >= 2) | (Decision == "NO-GO")) ~ "NO-GO",
                              TRUE ~ "Continue")) %>% 
  mutate(decision = factor(decision, levels = c("NO-GO", "Continue", "GO"))) %>% 
...
```{r fig.height=10, fig.cap="Interim analysis decision probabilities for simulated arms with TTP slope relative to the control as specified in the panels (all corresponding to the `1 Winner` simulation setting) and unfavorable outcome rates of A) 10% (unfavorable), B) 5% (minimal), and C) 2.5% (desirable). Arms are flagged for a GO decision if they meet all the following conditions: fewer than 2 unfavorable outcomes, evidence of meeting the target product profile, and a posterior probability greater than 50% of ranking in the top two arms. A pattern is applied to differentiate the criterion responsible for NO-GO decisions. Arms are flagged for a NO-GO decision if they experience any of the following conditions: 2 or more unfavorable outcomes (stripes), do not have evidence of meeting the target product profile (dots), or meet both conditions (plain). All arms that do not meet the criteria for a GO or NO-GO decision, receive a “Continue” designation. Results are based on 1,000 simulated datasets per setting."}
p_suboptimal / p_minimal / p_desirable + 
  plot_layout(guides = "collect") + 
  plot_annotation(tag_levels = "A") &
  theme(legend.position = "bottom")
...
```{r}
#| eval: false
ggsave(filename = here("graphs", "submission", "fig_panel-oc-full-framework_1-winner.png"),
       device = "png",
       height = 8,
       width = 8,
       units = "in")
...
d20 <- data_shape_function(relapse_data = s95.sit1.20,
                           relapse_regimen = "2", # Sub-optimal
                           tpp_data = decisions$decisions_high,
                           ranking_data = ranking_probabilities$high_probabilities,
                           true_slopes = c("35%", "37%", "39%", "41%"),
                           nk_choice = 20)
d30 <- data_shape_function(relapse_data = s95.sit1.30, 
                           relapse_regimen = "2", # Sub-optimal
                           tpp_data = decisions$decisions_high,
...
```{r fig.height=10, fig.cap="Interim analysis decision probabilities for simulated arms with TTP slope relative to the control as specified in the panels (all corresponding to the `4 Winners` simulation setting) and unfavorable outcome rates of A) 10% (unfavorable), B) 5% (minimal), and C) 2.5% (desirable). Arms are flagged for a GO decision if they meet all the following conditions: fewer than 2 unfavorable outcomes, evidence of meeting the target product profile, and a posterior probability greater than 50% of ranking in the top two arms. A pattern is applied to differentiate the criterion responsible for NO-GO decisions. Arms are flagged for a NO-GO decision if they experience any of the following conditions: 2 or more unfavorable outcomes (stripes), do not have evidence of meeting the target product profile (dots), or meet both conditions (plain). All arms that do not meet the criteria for a GO or NO-GO decision, receive a “Continue” designation. Results are based on 1,000 simulated datasets per setting."}
p_suboptimal / p_minimal / p_desirable + 
  plot_layout(guides = "collect") + 
  plot_annotation(tag_levels = "A") &
  theme(legend.position = "bottom")
...
```{r}
#| eval: false
ggsave(filename = here("graphs", "submission", "fig_panel-oc-full-framework_4-winners.png"),
       device = "png",
       height = 8,
       width = 8,
       units = "in")
...
d20 <- data_shape_function(relapse_data = s95.sit1.20,
                           relapse_regimen = "2", # Sub-optimal
                           tpp_data = decisions$decisions_null,
                           ranking_data = ranking_probabilities$null_probabilities,
                           true_slopes = c("0%", "0% ", " 0%", " 0% "),
                           nk_choice = 20)
d30 <- data_shape_function(relapse_data = s95.sit1.30, 
                           relapse_regimen = "2", # Sub-optimal
                           tpp_data = decisions$decisions_null,
...
```{r fig.height=10, fig.cap="Interim analysis decision probabilities for simulated arms with TTP slope relative to the control as specified in the panels (all corresponding to the `No Winners` simulation setting) and unfavorable outcome rates of A) 10% (unfavorable), B) 5% (minimal), and C) 2.5% (desirable). Arms are flagged for a GO decision if they meet all the following conditions: fewer than 2 unfavorable outcomes, evidence of meeting the target product profile, and a posterior probability greater than 50% of ranking in the top two arms. A pattern is applied to differentiate the criterion responsible for NO-GO decisions. Arms are flagged for a NO-GO decision if they experience any of the following conditions: 2 or more unfavorable outcomes (stripes), do not have evidence of meeting the target product profile (dots), or meet both conditions (plain). All arms that do not meet the criteria for a GO or NO-GO decision, receive a “Continue” designation. Results are based on 1,000 simulated datasets per setting."}
p_suboptimal / p_minimal / p_desirable + 
  plot_layout(guides = "collect") + 
  plot_annotation(tag_levels = "A") &
  theme(legend.position = "bottom")
...
```{r}
#| eval: false
ggsave(filename = here("graphs", "submission", "fig_panel-oc-full-framework_0-winners.png"),
       device = "png",
       height = 8,
       width = 8,
       units = "in")
