...
data_shape_function <- function(relapse_data, tpp_data, ranking_data, nk_choice){
  # Relapses
  r1 <- relapse_data %>% # Situation 1: Regimen 2 - Sub-optimal
    filter(ttp.condition == "highlow", regimen == "2") %>%  # TTP did not play a role in simulating relapse data. Selection of ttp condition is irrelevant to the relapses counted. However, a unique relapse dataset was generated for each ttp condition so we will take advantage of this by pulling out the sub-optimal relapse counts from each dataset.
    dplyr::select(-regimen, -ttp.condition)
  
  r2 <- relapse_data %>% # Situation 1: Regimen 2 - Sub-optimal
    filter(ttp.condition == "high", regimen == "2") %>%  # TTP did not play a role in simulating relapse data. Selection of ttp condition is irrelevant to the relapses counted. However, a unique relapse dataset was generated for each ttp condition so we will take advantage of this by pulling out the sub-optimal relapse counts from each dataset.
    dplyr::select(-regimen, -ttp.condition)
  
  r3 <- relapse_data %>% # Situation 1: Regimen 2 - Sub-optimal
    filter(ttp.condition == "even", regimen == "2") %>% 
    dplyr::select(-regimen, -ttp.condition)
  ...
  out1 <- full_join(
    ...
    mutate(true.slope = "-10%",
           ...
           nk = nk_choice)
  out2 <- full_join(
    full_join(r2, tpp2), rank2) %>% 
    mutate(true.slope = "10%",
           true.relapse = "sub-optimal",
           nk = nk_choice)
  out3 <- full_join(
    full_join(r3, tpp3), rank3) %>% 
...
p_suboptimal <- bind_rows(d20, d30, d40) %>% #, d60, d80) %>% 
  mutate(decision = case_when((n_relapse_interim1 < 2 & Decision == "GO") ~ "GO",
                              ((n_relapse_interim1 >= 2) | (Decision == "NO-GO")) ~ "NO-GO",
                              TRUE ~ "Continue")) %>% 
  mutate(decision = factor(decision, levels = c("NO-GO", "Continue", "GO"))) %>% 
  group_by(true.slope, nk, decision) %>% 
  summarise(n = n()) %>% 
...
ggsave(filename = here("graphs", "simulation-results", 
                       paste0(Sys.Date(), "_U4TB-decision-criteria-oc.png")),
       device = "png",
       height = 4,
       width = 8,
       units = "in")
...
```{r}
data_shape_function_desirable <- function(relapse_data, tpp_data, ranking_data, nk_choice){
  # Relapses
  r1 <- relapse_data %>% # Situation 1: Regimen 5 - Desirable
    filter(ttp.condition == "highlow", regimen == "5") %>%  # TTP did not play a role in simulating relapse data. Selection of ttp condition is irrelevant to the relapses counted. However, a unique relapse dataset was generated for each ttp condition so we will take advantage of this by pulling out the Desirable relapse counts from each dataset.
    dplyr::select(-regimen, -ttp.condition)
  
  r2 <- relapse_data %>% # Situation 1: Regimen 5 - Desirable
    filter(ttp.condition == "high", regimen == "5") %>%  # TTP did not play a role in simulating relapse data. Selection of ttp condition is irrelevant to the relapses counted. However, a unique relapse dataset was generated for each ttp condition so we will take advantage of this by pulling out the Desirable relapse counts from each dataset.
    dplyr::select(-regimen, -ttp.condition)
  
  r3 <- relapse_data %>% # Situation 1: Regimen 5 - Desirable
    filter(ttp.condition == "even", regimen == "5") %>% 
    dplyr::select(-regimen, -ttp.condition)
...
d20_desirable <- data_shape_function_desirable(relapse_data = s95.sit1.20,
                                     tpp_data = decisions$decisions_highlow,
                                     ranking_data = ranking_probabilities$highlow_probabilities,
                                     nk_choice = 20)
d30_desirable <- data_shape_function_desirable(relapse_data = s95.sit1.30, 
                                     tpp_data = decisions$decisions_highlow,
                                     ranking_data = ranking_probabilities$highlow_probabilities,
                                     nk_choice = 30)
d40_desirable <- data_shape_function_desirable(relapse_data = s95.sit1.40, 
                                     tpp_data = decisions$decisions_highlow,
                                     ranking_data = ranking_probabilities$highlow_probabilities,
                                     nk_choice = 40)
d60_desirable <- data_shape_function_desirable(relapse_data = s95.sit1.60, 
                                     tpp_data = decisions$decisions_highlow,
                                     ranking_data = ranking_probabilities$highlow_probabilities,
                                     nk_choice = 60)
d80_desirable <- data_shape_function_desirable(relapse_data = s95.sit1.80, 
                                     tpp_data = decisions$decisions_highlow,
                                     ranking_data = ranking_probabilities$highlow_probabilities,
                                     nk_choice = 80)

p_desirable <- bind_rows(d20_desirable, d30_desirable, d40_desirable) %>% #, d60, d80) %>% 
  mutate(decision = case_when((n_relapse_interim1 < 2 & Decision == "GO" & p.subset.2 > 0.8) ~ "GO",
                              ((n_relapse_interim1 >= 2) | (Decision == "NO-GO") & p.subset.2 < 0.6) ~ "NO-GO",
                              TRUE ~ "Continue")) %>% 
  mutate(decision = factor(decision, levels = c("NO-GO", "Continue", "GO"))) %>% 
  group_by(true.slope, nk, decision) %>% 
  summarise(n = n()) %>% 
