...
######################
# relapses
out_hl_30_relapse_r1 <- s95.sit1.30 %>% 
  filter(ttp.condition == "highlow", regimen == "2") %>%  # even though TTP did not play a role in simulating relapse data, a unique relapse dataset was simulated for each unique TTP data (essentially resulting in 3,000 simulated datasets of relapse for each regimen for each condition). 
  dplyr::select(-regimen)
out_hl_30_relapse_r2 <- s95.sit1.30 %>% 
  filter(ttp.condition == "highlow", regimen == "2") %>%  # even though TTP did not play a role in simulating relapse data, a unique relapse dataset was simulated for each unique TTP data (essentially resulting in 3,000 simulated datasets of relapse for each regimen for each condition). 
  dplyr::select(-regimen)
...
out_hl_30_tpp_r1 <- decisions$decisions_highlow %>% 
  filter(Regimen == "Regimen 2", nk == 30) %>% 
  dplyr::select(-Regimen)
out_hl_30_tpp_r2 <- decisions$decisions_highlow %>% 
  filter(Regimen == "Regimen 3", nk == 30) %>% 
  dplyr::select(-Regimen)
