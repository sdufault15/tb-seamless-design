...
p1 <- ggplot() + 
  ...
  geom_polygon(aes(x = 0.75+1*cos(seq(0,2*pi,length.out=100)),
               ...
               fill = ucsf_col_pal[3],
               alpha = 0.3) + 
  annotate(geom = "text", x = 1.5, y = 3, hjust = 0, label = "Relapse", size = 5) + 
...
t2 <- out_hl_30_r2 %>% # THINK THIS IS THE PROBLEM
  summarise(`Stop relapse` = mean(n_relapse_interim1 >= 2),
            `Stop TPP` = mean(Decision == "NO-GO" | Decision == "Continue"),
            `P(top 2) < 0.6` = mean(p.subset.2 < 0.6),
            `Stop relapse or TPP` = mean((n_relapse_interim1 >= 2) | (Decision == "NO-GO") | (Decision == "Continue")),
...
```{r fig.width = 13, fig.height = 5, fig.cap = "Frequentist summary of estimator performance across changes in sample size (nk) and differences in $\\log_{10}$(TTP) slope. For all panels, results are based on 1,000 simulated datasets were used for each sample size and condition. A) The proportion of simulations where a given regimen was estimated to have the steepest slope. B) The proportion of simulations where the steepest estimated slope belonged to one of the true top two steepest regimens. C) The proportion of simulations (y-axis) where the estimated control slope falls outside of the estimated 95\\% confidence interval for a regimen with a given effect size (x-axis)."}
