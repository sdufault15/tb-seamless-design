...
decisions <- mcmc_mods_highlow_30 %>% 
  map_dfr(
    # Convert to % relative to control
    ~mutate(.x, `Regimen 1` = ((arm2/arm1) - 1)*100,
            `Regimen 2` = ((arm3/arm1) - 1)*100,
...
decisions_40 <- mcmc_mods_highlow_40 %>% 
  map_dfr(
    # Convert to % relative to control
    ~mutate(.x, `Regimen 1` = ((arm2/arm1) - 1)*100,
            `Regimen 2` = ((arm3/arm1) - 1)*100,
...
decisions_20 <- mcmc_mods_highlow_20 %>% 
  map_dfr(
    # Convert to % relative to control
    ~mutate(.x, `Regimen 1` = ((arm2/arm1) - 1)*100,
            `Regimen 2` = ((arm3/arm1) - 1)*100,
...
```{r barplot_decision-probabilities_even_varying-lrv, fig.width = 6, fig.height = 6}
...
```{r barplot_decision-probabilities_even_varying-tv, fig.width = 6, fig.height = 6}
