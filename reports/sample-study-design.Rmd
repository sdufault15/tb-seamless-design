...
```{r initial-data-setup, eval = FALSE}
# REMOVE
...
df_outcome <- df_outcome %>% 
  ...
  map(~mutate(.x, regimen = factor(regimen, levels = 1:5)))
```
...
# Join Relapse Data
########################
df_outcome_long <- df_outcome %>% 
  map(~full_join(.x, df_temp_long)) %>% 
  map(~mutate(.x, 
              observed.relapse = ifelse(status == 1 & eventtime <= 52 - duration, 1, 0),
              observed.relapse.date.trial = ifelse(observed.relapse == 1, eventtime + duration + enroll.day/7, NA)))

head(df_outcome_long)
```

```{r}
df_outcome_long$s97.situation1 %>%
  dplyr::select(`Patient ID` = patient.id, 
                `Day of enrollment (tau)` = enroll.day, 
                Regimen = regimen, 
                Duration = duration, 
                `Weeks since randomization (t)` = patient.visit.week, 
                `log10(TTP)` = yij_censored, 
                `Relapse?` = observed.relapse, 
                `Day of relapse (tau)` = observed.relapse.date.trial) %>% 
  head(n = 45) %>%
  group_by(`Patient ID`) %>%
  slice_head(n = 3) %>%
  arrange(`Day of enrollment (tau)`) %>%
  kable(caption = "First 3 observations for first 5 patients enrolled.",
        digits = 2) %>%
  kable_styling() %>% 
...
#### Interim 1
**Need to decide how big to "oversample" the control arm**

At an enrolment rate of 10 participants per day, with 5 regimens and 5 durations per experimental regimens, the initial interim analysis will require:

\[\begin{align}
N_{\text{interim 1}} &= 4 \text{ experimental regimens} \times 30 \text{ participants per regimen} \\ 
&\hspace{1em} + 1 \text{ control regimen}\times30 \text{ participants (minimum)} \times C \\
&\geq 150 \text{participants}
\end{align}\]

The first interim analysis will not be able to be carried out until (AT MINIMUM) 27 weeks after the first day of enrollment (15 weeks of enrollment + 8 weeks of follow-up visits + $\approx$ 4 weeks for observation of TTP for last sputum sample). However, because we are concurrently randomizing to regimen and duration, it may take longer. *This depends on whether we care about balance in the duration sub-strata.*

Here we trigger the first interim analysis $\approx 4$ weeks after the last sputum sample is collected on the last participant and 30 participants have been enrolled in each regimen.

```{r}
interim1 <- df_outcome_long$s97.situation1 %>%  # same for all situations
  dplyr::select(patient.id, enroll.day, patient.visit.week, regimen, duration) %>% 
  # First, pull their last entry
  group_by(patient.id) %>% 
  filter(patient.visit.week == max(patient.visit.week)) %>%
  distinct() 
...
interim1 <- interim1 %>%
  arrange(enroll.day) %>%
  group_by(regimen) %>%
  mutate(nk_enroll = row_number()) %>%
  ungroup() %>%
  filter(nk_enroll == 30) %>%
  mutate(last.visit.trial = patient.visit.week + enroll.day/7) %>%
  summarise(last.visit.trial = max(last.visit.trial))

print(interim1)

df_interim1 <- map_dfr(df_outcome_long,
                       ~group_by(.x, patient.id) %>% 
                         filter(max(patient.visit.week) + enroll.day/7 <= (interim1$last.visit.trial)*(7.5/7)) %>% 
                         mutate(relapse.interim1 = ifelse(observed.relapse == 1 & observed.relapse.date.trial <= interim1$last.visit.trial + 4, 1, 
...
```{r eval = FALSE}
save(df_interim1,
     file = here("sandbox", paste0(Sys.Date(), "_interim1-data-example.RData")))
```

```{r eval = FALSE}
df_interim1 %>%
  filter(situation == "s97.situation1") %>%
  ggplot(aes(x = patient.visit.week, y = 10^yij_censored)) +
  geom_smooth(aes(col = regimen)) +
  scale_y_log10("TTP, log10 scale") +
  scale_x_continuous("Weeks since randomization") +
  theme_bw()

relapse_output_interim <- df_interim1 %>%
  group_by(situation, regimen, duration, relapse.interim1) %>%
  summarise(n = n_distinct(patient.id))

relapse_output_interim %>%
  ungroup() %>%
  filter(situation == "s97.situation1") %>%
  dplyr::select(-situation) %>%
  mutate(relapse.interim1 = ifelse(relapse.interim1 == 1, "Relapse", "No relapse")) %>%
  pivot_wider(names_from = relapse.interim1,
              values_from = n,
              values_fill = ) %>%
  kable() %>%
  kable_styling()
...
![Example diagram of data for first interim analysis](../graphs/exploratory/interim-1-visualization.png)

```{r}
df_interim1 %>% head()
  # group_by(regimen) %>% 
  # summarise(n = n_distinct(patient.id)) %>% 
  # kable(caption = "Number included per regimen in the interim analysis.") %>% 
  # kable_styling()
```
