...
# All together
###################
full_join(full_join(surv.k2.d16, surv.k3.d16), surv.k5.d16) %>% 
  ...
  pivot_longer(cols = c(1,3:5),
               values_to = "survival",
               names_to = "regimen") %>% 
  mutate(`Relapse rate` = case_when(regimen == "Regimen 2" ~ "Sub-optimal",
                                    regimen == "Regimen 3" ~ "Minimal",
                                    regimen == "Regimen 5" ~ "Desirable",
                                    TRUE ~ regimen)) %>% 
  ggplot(aes(x = t, y = survival,lty = `Relapse rate`,
             col = `Relapse rate`)) + 
  geom_line(lwd = 1) + 
  theme_bw() + 
  ylab("S(t)") +
  scale_x_continuous("Time since randomization (months)",
                     ...
                     labels = c(0,3,6,9,12)) +
  coord_cartesian(xlim = c(0,52), ylim = c(0.8,1)) +
  scale_color_manual(values = c("turquoise4", rep("black",3))) +
  # ggtitle("",
  #         subtitle = "Control relapse rate set at 5% by end of follow-up") + 
  theme_bw() +
...
ggsave(filename = here("graphs", "simulation-settings", 
                       paste0(Sys.Date(),"_survival-curve-general.jpg")),
       device = "jpg",
       width = 6,
       height = 8,
       units = "in")
